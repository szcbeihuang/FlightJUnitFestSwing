...
public class HMenuAction implements ActionListener{
	...
	public static final String strRemoveCompanyFlight = "删除航空公司的所有航班";
	...
	public void actionPerformed(ActionEvent e) {
		...
		else if( strRemoveCompanyFlight==strMenu )//删除航空公司的所有航班
		{
			DelCompanyAllFlight();
		}
	...
	private void DelOneFlight()
	{
		...
		{ //strName=null按的取消按钮。
			...
			if( true==bResult )//成功删除
				...
				JOptionPane.showMessageDialog(null,
						"<html><body><h1>该航班名称不存在，注意与航班号和航空公司名称区分开来！</h1></body></html>",
						"删除一个航班",JOptionPane.PLAIN_MESSAGE);
		}
	}

	//删除航空公司的所有航班
	private void DelCompanyAllFlight()
	{
		String strName = JOptionPane.showInputDialog(null, "请输入航空公司名称","删除航空公司的所有航班" , JOptionPane.INFORMATION_MESSAGE);
		if( null!=strName )
		{ //strName=null按的取消按钮。
			boolean bResult = m_hmfDmana.DelCompanyFlight(strName);
			if( true==bResult )//成功删除
				ShowAllFlight(m_hmfDmana.GetAllFlightInfo());
			else
				JOptionPane.showMessageDialog(null,
						"<html><body><h1>该航空公司不存在，注意与航班号和航班名称区分开来！</h1></body></html>",
						"删除航空公司的所有航班",JOptionPane.PLAIN_MESSAGE);
		}
	}
	...
	{
		JOptionPane.showMessageDialog(null,
				"<html><body><h1>学习调试程序技能教学程序</h1><h3>参看实验指导书</h3></body></html>",
