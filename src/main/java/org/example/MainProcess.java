...
public class MainProcess extends JFrame{
	...
	protected static JMenuBar AddMenu(HMenuAction hmaAction)
	{
		...
		JMenuItem jmiRemoveCompanyAll; //为调试工程单独做的菜单，何海江，其它工程没有该菜单项目。
		...
		jmiRemoveCompanyAll = new JMenuItem(HMenuAction.strRemoveCompanyFlight);
		jmiRemoveCompanyAll.addActionListener(hmaAction );// 菜单添加事件
		...
		mnuFile.add(jmiRemoveCompanyAll);
		//将菜单加入至菜单栏
		jmbWork.add(mnuFile);
		//查询统计
